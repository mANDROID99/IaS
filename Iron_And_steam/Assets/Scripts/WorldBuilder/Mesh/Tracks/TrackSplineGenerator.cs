...
namespace IaS.WorldBuilder.Meshes.Tracks
{
    public class TrackSplineGenerator
    {
        ...
        public BezierSpline[] GenerateSplines(SplitTrack track, SubTrack subTrack)
        {
            ...
            for (int groupIdx = 0; groupIdx < subTrack.trackNodes.Length; groupIdx ++ )
            {
                SubTrackNode[] subTrackNodes = subTrack.trackNodes[groupIdx];
                if (subTrackNodes.Length < 2)
                    continue;
                ...
                for(int i = 0; i < subTrackNodes.Length - 1; i++)
                {
                    SubTrackNode current = subTrackNodes[i];
                ...
                splines.Add(new BezierSpline(pts.ToArray()));
            }
