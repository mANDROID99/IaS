...
namespace IaS.Domain
{
    public class BlocksCreator
    {
        ...
        {
            BlockBounds[] splitRegions = groupBranch.RotateableBounds.ToArray();
            ...
            foreach (MeshBlock block in splitBlocks)
            {
                ...
                RotateableBranch rotateableBranch = groupBranch.GetRotateableBranch(region);

                AttachGameObject(block, splitBlocks, splits, rotateableBranch.BlocksLeaf, prefabs.BlockPrefab, adjacencyCalculator);
            }
        }
